...
simplex2(int n, const double *eps, double size_limit,
		 ...
		 double *x, double *fn_ret, void *data, int *steps_ret)
 ...
 * the simplex are smaller than 'size_limit', or after 1000 iterations
 * (whatever comes first).
 *
 * On return the vector 'x' contains the best found solution, that is,
 * the parameters with the smallest result of 'fn'.  If 'fn_ret' is
 * not NULL, the result of 'fn' applied to 'x' is copied into this
 * vector.  If 'steps_ret' is not NULL, the number of optimisation
 * steps_ret performed is stored in '*steps_ret'.
 */
{
	double *points;
