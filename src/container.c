...
struct set *
cpy_set (struct set * orig)
{
	int i;
	struct set *set;
	set = xnew(struct set, 1);
	set->used = orig->used;
	set->alloc = orig->alloc;
	set->item = xnew(char *, set->alloc);
	
	for (i = 0; i < set->used; ++i) {
		set->item[i] = xstrdup(orig->item[i]);
	}
 ...
 * corresponding entry in set is returned.  If the element is not
 * present but 'add' is true, a copy of 'item' is added to 'set' and
 * the index of the newly added element is returned.  Otherwise -1 is
 * returned.
 */
int
